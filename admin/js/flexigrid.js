...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 disableSelect: false,
			 dblclickFunction: null  //when initiating flexigrid specify the dblClick function name 
		  }, p);
		...
		var g = {
			...
			populate: function () { //get latest data
				...
				var param = {page:p.newp, rp: p.rp, sortname: p.sortname, sortorder: p.sortorder, query: p.query, qtype: p.qtype, busqExac: p.busqExac};
			...
			doSearch: function () {
				...
				p.busqExac = $('#busquedaExacta').is(':checked');
				p.newp = 1;
				this.populate();				
			},
		...
		if (p.searchitems)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'>Quick Search <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"+sopt+"</select> <input type='checkbox' value='Y' id = 'busquedaExacta' name = 'busquedaExacta' />Búsqueda exacta <input type='button' value='Search' id = 'btnSearch' /><input type='button' value='Clear' /></div>");

				$('input[name=q],select[name=qtype],checkbox[name=busquedaExacta]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch()});
				$('#btnSearch',g.sDiv).click(function(){g.doSearch();});
