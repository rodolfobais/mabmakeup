...
$sql = "
	INSERT INTO `misservicios`(`fecha`, `imagen`, `comentario`) 
	VALUES ('".$date."','','".$_POST['comentario']."');";
$arr = $db -> QuerySimple($sql);

$sql = "SELECT MAX(`id`) FROM `misservicios`;";
...
// 	echo "ext: ".$extension;
	$nombre = "misservicios_".$id.".".$extension;
...
// 	echo "fn: ".move_uploaded_file($_FILES['imagen']['tmp_name'], 'uploads/'.$_FILES['imagen']['name']);
	if(move_uploaded_file($_FILES['imagen']['tmp_name'], '../../../gallery/'.$nombre)){
		$sql = "UPDATE `misservicios` SET `imagen` = '".$nombre."' WHERE id = '".$id."'";
		$arr = $db -> QuerySimple($sql);
		header('Location: /mabmakeup/admin/index.php?menu=misservicios');
		echo '{"status":"success"}';
		die;
	}else{
		$sql = "DELETE FROM `misservicios` WHERE id = '".$id."'";
		$arr = $db -> QuerySimple($sql);
